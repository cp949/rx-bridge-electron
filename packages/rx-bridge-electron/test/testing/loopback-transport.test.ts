...
// 왕복·microtask 순서·dispose 후 동작·sender 분리·server throw 노출을 다룬다.
...
type AppBridge = {
  device: {
    rpc: { echo(input: EchoInput): EchoOutput };
    state: { count: CountState };
    event: { tick: TickEvent };
  };
};

/** 매 test가 쓰는 최소 impl 하나. RPC echo·State count·Event tick 셋을 노출한다. */
function buildImpl(): {
...
describe("createLoopbackTransport: clone 경계", () => {
  test("함수가 포함된 RPC input은 structuredClone DataCloneError로 reject된다", async () => {
    const { impl } = buildImpl();
    const server = createBridgeServer(impl);
    const transport = createLoopbackTransport(server);
