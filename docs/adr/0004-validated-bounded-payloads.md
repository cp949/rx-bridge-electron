...
> 개정(RD-049): 아래 둘째 문단의 "Electron 어댑터와 preload는 envelope 구조만 검사" 서술은 [ADR 0016](0016-sender-admission.md) 결정 2 이후 어댑터에 맞지 않다. 어댑터(`electron-adapter.ts`)는 envelope를 parse하지 않고 받은 값을 server에 넘긴다. envelope parse는 server가 한다. preload는 계속 envelope 구조와 값 프로필을 검사하되 크기 한도는 없다(`src/protocol/messages.ts`의 `ENVELOPE_LIMITS`). payload 한도를 server 한 곳에서만 강제한다는 결정은 그대로다.
