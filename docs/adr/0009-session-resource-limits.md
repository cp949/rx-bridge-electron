...
11. **구독 계산**: 대기(`authorize` 중)와 활성을 합쳐 `maxSubscriptions` 하나로 센다. unsubscribe·완료·오류·overflow·거부(`FORBIDDEN`/`NOT_FOUND`/`authorize` 예외)·세션 retire 각 경로 뒤 슬롯을 즉시 반환한다. 초과 시 `streams.reject()`로 `subscribed` 다음 `RESOURCE_EXHAUSTED` `error`를 보낸다.
...
서버 전역(모든 세션 합계) 상한, Renderer 쪽 사전 차단, 와이어 `timeoutMs`, RPC `requestId` 워터마크, IPC 역직렬화 단계의 수신 메모리 제한, 진단 지표 추가(RD-007), 실제 Electron 다중 창·장시간 검증(RD-008).
