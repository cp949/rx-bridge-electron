...
_(개정: RD-049 — 3번의 RPC별 순서는 "cancel 전송 → reject"가 아니라 "확정 선점(`rpc-settled` 진단 기록) → 이미 전송된 요청이면 cancel 전송 → reject"다(`rpc-client.ts`의 `cancelOnce`). 확정을 cancel 전송보다 먼저 선점해야 `transport.cancel`이 같은 호출의 abort·dispose를 동기로 재진입시켜도 먼저 발생한 원인이 이기고 cancel이 한 번만 나간다. 아직 전송되지 않은 요청에는 cancel을 보내지 않는다.)_
...
_(개정: RD-049 — 위 목록과 이 문단의 `undefined` 반환 서술은 [ADR 0016](0016-sender-admission.md) 이후 맞지 않다. `establish()`·`current()`는 `Admission`(`{ session }` 또는 `{ reason }`)을 반환하고, 종료 뒤에는 `{ reason: "sender-unauthorized" }`다. handshake 거부 응답은 adapter의 `protocolError` 폴백이 아니라 `server.handshake`가 사유를 진단에 기록한 뒤 직접 반환하는 `INVALID_ARGUMENT "Invalid bridge request."`다. adapter의 catch는 Electron 객체 접근 예외만 막는다. stream subscribe는 무시되지 않고 [ADR 0020](0020-stream-terminal-on-retire.md)대로 `subscribed`(0) 뒤 `error FORBIDDEN`을 받는다. RPC 응답(`FORBIDDEN "Bridge sender is not authorized."`)은 그대로다.)_
...
`dispose()`를 호출하지 않아도 navigation·창 파괴 시 Main이 세션을 이미 retire하므로, hello-world 연결 설정에서 `dispose()`를 자동 호출하는 `pagehide` 등록은 불필요하다 — `dispose()`는 브리지 생존 중 SPA teardown 용도로 쓴다. 근거는 [ADR 0013](0013-wiring-defaults.md)에 있다.
