...
RPC 수명주기(`tryAcquireRpc`·`beginRpc`·`finishRpc`·`releaseRpc`·`cancelRpc`, RPC `keyOf`) — 후속 ROADMAP.md#RD-016. sender admission 통합, `recordAdapterRejection` Symbol, version-mismatch 도달 불가 분기. 와이어 형식·채널·handshake·공개 export 변경(`Subscriptions`는 공개 export가 아니다 — `StreamHub`와 마찬가지로 내부 구현이다).
