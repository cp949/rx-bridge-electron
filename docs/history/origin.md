...
이 문서는 이전 Git 저장소의 작업 기준과 주요 커밋을 새 Git 이력이 시작된 뒤에도 추적할 수 있도록 남긴다. 코드 동작의 근거는 현재 코드와 [아키텍처 문서](../architecture.md), 미래 작업은 루트 `ROADMAP.md`를 따른다. 아래 커밋 제목은 변경 의도를 찾기 위한 인덱스이며, diff나 테스트 로그를 대신하지 않는다.
