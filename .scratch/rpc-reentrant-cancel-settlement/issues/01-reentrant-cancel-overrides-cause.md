...
`packages/rx-bridge-electron/test/renderer/rpc-calls.test.ts:191` "sends cancel once when transport.cancel re-enters the call's abort"는 cancel 1회와 확정 1회만 단언하고 error `code`는 단언하지 않는다. 동작을 정하면 이 test에 `code` 단언을 추가한다.
